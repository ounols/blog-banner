...
void main(void) {
    ...
    for(int i = 0; i < 1; ++i) {
        ...
        vec4 directionLight = u_lightPosition[0];
        ...
        // Varying
        v_lightPosition[0] = normalize(aux);
        v_distance[0] = length(aux);
        // Varying Shadow
        if (u_shadowMode[0] == 1) {
            v_fragPosLightSpace[0] = u_lightMatrix[0] * lightPurePosition;
        } else {
            v_fragPosLightSpace[0] = vec4(c_zero, c_zero, c_zero, c_one);
        }
    }

...
void main(void) {
	...
	for(int i = 0; i < 1; ++i) {
		...
		vec3 V = (u_lightType[0] > 1) ? V0 : N;
		vec3 L = v_lightPosition[0];
		vec3 H = normalize(V + L);
		float distance = v_distance[0];
		float attenuation = 1.0 / (distance * distance);
		float shadow = 0.0;
		if(u_shadowMode[0] == 1) {
			shadow = ShadowCalculation(0, v_fragPosLightSpace[0], N, L);
		}
		vec3 radiance = (u_lightColor[0] * attenuation) * (1.0 - shadow);
		// Cook-Torrance BRDF

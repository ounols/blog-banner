...
{
	highp vec2 normal_raw = texture(u_sampler_normal, v_textureCoordOut).xz;
	normal_raw = vec2(normal_raw.x, normal_raw.y);
	highp vec3 tangentNormal = vec3(normal_raw.x, max(sqrt(1.f - normal_raw.x * normal_raw.x - normal_raw.y * normal_raw.y), 0.f), normal_raw.y);
	tangentNormal = normalize(tangentNormal) * 2.0 - 1.0;

	vec3 Q1  = dFdx(v_worldPosition);
	vec3 Q2  = dFdy(v_worldPosition);
	vec2 st1 = dFdx(v_textureCoordOut);
	vec2 st2 = dFdy(v_textureCoordOut);
...
void main(void) {
	...
	highp float metallic  = 0.1;
	highp float roughness = 0.9;
...
//			++index_shadow;
//		}
		vec3 radiance = (u_lightColor[i] * 0.7 * attenuation) * (1.0 - shadow);
		// Cook-Torrance BRDF
